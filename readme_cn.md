...
- **🤖 Telegram 集成**：通过 Telegram Bot 随时随地监控、控制脚本并查看日志
...
Python Manager 支持通过 Telegram Bot 进行远程管理。

### 配置

在 `config.json` 中添加 `telegram` 配置部分：
...
```json
{
  "telegram": {
    ...
    "proxy_url": "http://127.0.0.1:7890" // 可选，国内使用需要配置代理
  }
}
```

- **token**: 从 @BotFather 获取的 Bot Token
- **allowed_chat_ids**: 允许使用 Bot 的用户 Chat ID 列表（为了安全，只有列表中的用户可以操作）
- **proxy_url**: 如果你的网络环境需要代理才能访问 Telegram，请配置此项
...
- **/list**: 显示交互式脚本列表
    ...
    - 点击脚本可查看详情并进行操作
    - 提供 **启动**、**停止**、**重启** 按钮
    - 提供 **📝 Logs** 按钮查看最近 10 行日志（自动过滤颜色代码）
- **/help**: 显示帮助信息
- **/run <id>**: 快速启动指定脚本
- **/stop <id>**: 快速停止指定脚本
- **/logs <id>**: 快速查看指定脚本日志
